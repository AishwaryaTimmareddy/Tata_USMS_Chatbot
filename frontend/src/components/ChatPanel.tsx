...
export function ChatPanel({ token }: { token: string }) {
  ...
  const [feedbackByMessage, setFeedbackByMessage] = useState<Record<string, "saving" | "helpful" | "notHelpful" | "error">>({});
  ...
  async function feedback(messageId: string, helpful: boolean) {
    ...
    try {
      await sendFeedback(token, sessionId, messageId, helpful);
    ...
    <div className="chat-layout">
      ...
      <div className="chat-columns">
        <aside className="history-card">
          <div className="history-heading"><History size={17} /><strong>Previous chats</strong></div>
          <button className="new-chat-button" onClick={reset}>+ New conversation</button>
          <div className="history-list">
            {conversations.map((conversation) => (
              <button
                className={conversation.session_id === sessionId ? "history-item active" : "history-item"}
        ...
        <div className="chat-topbar">
          ...
          <div>
            ...
            <span><i /> Signed-in user</span>
          </div>
          <div className="chat-trust"><Check size={14} /> Approved sources only</div>
        </div>
              ...
              <div className="message-content">
                      ...
                      {message.citations.map((citation, index) => (
                        <div className="citation" key={`${citation.source}-${index}`}>
                          ...
                          <div>
                            ...
                            {citation.page_number && <small>Page {citation.page_number}</small>}
                          </div>
                        </div>
                      ))}
                    </div>
                ...
                {message.role === "assistant" && message.id !== "welcome" && (
                  <div className="message-meta">
                    ...
                    {(feedbackByMessage[message.id] === "helpful" || feedbackByMessage[message.id] === "notHelpful") && (
